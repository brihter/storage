import { Node, SyntaxKind, ParameterDeclaration, SourceFile, Type } from 'ts-morph'
...
const getFunction = (node: Node): TypeInfo | undefined => {
    ...
    const description = getDescription(node)
    const parameters = node.getParameters().map(getParametersSignature)
    const returnType = node.getReturnType().getAliasSymbol()?.getEscapedName() || 'unknown'
    const callSignatures = parameters.map(p => `${name}(${p?.name}: ${p?.returnType}): ${returnType}`)
    const example = getExample(node)
